...
export default function boardReducers(
  ...
  switch (action.type) {
    case TypeKeys.updateControlValue:
      ...
      const controlIdx = currentBoard.controls.findIndex(
        (ctrl) => ctrl.blockId === blockId && ctrl.paramId === paramId
      );
      const updatedCCMap = { ...currentBoard.ccMap };
      const controls = currentBoard.controls.map((ctrl, i) => {
        if (i === controlIdx) {
          const controlObj = { ...ctrl, paramValue, formattedValue };
          updatedCCMap[ctrl.cc] = controlObj;
          return controlObj;
        }
        return ctrl;
      });
    ...
    case TypeKeys.setBoard:
      const existingBoardIdx = state.boards.findIndex(
      ...
      if (existingBoardIdx !== -1) {
        ...
        return {
          ...
          boards: state.boards.map((board, i) =>
            i === existingBoardIdx ? { ...board, ...payload } : board
          ),
        };
      } else {
        // Add new board
