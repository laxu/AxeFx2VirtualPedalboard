...
export default class ControlEditorComponent extends React.Component<
  Props,
  State
> {
  constructor(props) {
  ...
  checkValidity() {
    ...
    this.setState({
      isValid:
        blockId > 0 && blockId < NO_VALUE && paramId >= 0 && paramId < NO_VALUE
    });
  }
    ...
    return (
      ...
      >
        ...
        <div className="form-group">
          ...
          <select
            value={(blockId && blockId.toString()) || NO_VALUE}
            onChange={event => this.setValue("blockId", event.target.value)}
          >
            <option disabled value={NO_VALUE}>
              Select effects block
            </option>
            {blocks.map((block, i) => (
              <option key={`block-${i}`} value={block.id}>
                {block.label}
              </option>
            ))}
          </select>
        ...
        <div className="form-group">
          ...
          <select
            value={(paramId && paramId >= 0 && paramId.toString()) || NO_VALUE}
            disabled={!selectedBlock}
            onChange={event => this.setValue("paramId", event.target.value)}
          >
            <option disabled value={NO_VALUE}>
              {selectedBlock && selectedBlock.parameters.length === 0
                ? "Block is not yet supported"
                : "Select effects param"}
            </option>
            {parameterOptions.map((group, i) => (
              <optgroup label={group.label} key={`group-${i}`}>
                {group.parameters.map((param, j) => (
                  <option key={`param-${j}`} value={param.id}>
                    {param.label}
                  </option>
                ))}
              </optgroup>
          ...
          <div className="form-group--inline">
            <label className="label-radio">
              <input
                ...
                onChange={event =>
                  this.setValue("controlType", event.target.value)
                }
              />
              <span>Control</span>
            </label>
        ...
        <div className="form-group">
          <label>
            <input
              ...
              checked={isRelative}
              onChange={event =>
                this.setValue("isRelative", isRelative ? "" : "1")
              }
            />
